...
public class Subgraph {
	...
	public Subgraph(int order, boolean dir) {
		...
		if (directed)
			this.matrix = new AdjacencyMatrix(order, dir);
		else
			this.matrix = new AdjacencyMatrix(order);
	}
       ...
       /* matrix is different from directed and undirected. 
		...
		if (directed) {
			for (int i = 0; i < currentSize; i++) {
				...
				int opposit = (-1) * get(i);
				if (adjacencyList.contains(opposit))
	...
	private class AdjacencyMatrix {
		...
		boolean hasEdge(int x, int y) {
			if (directed)
				return x == y || matrix.get(indexForDir(x, y));
			else
		...
		byte[] toBytes() {
			...
			if (directed)
				currentIndex = prefix.length + orderBytes.length;
			byte currentByte = 0;
