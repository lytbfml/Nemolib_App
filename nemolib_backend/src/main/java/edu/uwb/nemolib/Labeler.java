...
public final class Labeler {
	...
	public Labeler() {
		...
		this.inputFilename = filePrefix + "rawgraph6_" + currentTime + filePostfix + instanceCounter;
		this.outputFilename = filePrefix + "canonical_" + currentTime + filePostfix + instanceCounter;
		this.args = getArgs();
	}
	...
	private Map<String, Set<Double>> g6toCanonical(Map<String,
		...
		Map<String, String> g6CanLabelMap = getCanonicalLabels(labelRelFreqsMap.keySet());
		Map<String, Set<Double>> result = new HashMap<>();
		for (Map.Entry<String, Set<Double>> labelRelFreqs : labelRelFreqsMap.entrySet()) {
	...
	public Map<String, String> getCanonicalLabels(Set<String> labels) {
		...
		if (returnCode != 0) {
			System.err.println("`labelg` exited with a return code of: " + returnCode);
	...
	private void deleteFile(String filename) {
		try {
			...
			System.err.println("Exception " + e + " raised when attempting to delete " + filename);
		}
	}
}
